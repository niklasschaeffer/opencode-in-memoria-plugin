...
class InMemoriaPlugin {
  ...
  async onProjectOpen(_context: ProjectContext): Promise<void> {
  ...
  async onToolExecuteBefore(ctx: ToolContext): Promise<ToolContext> {
    Logger.debug(`Tool executing: ${ctx.toolName}`)
    return ctx
  }

  async onToolExecuteAfter(ctx: ToolContext): Promise<ToolContext> {
    Logger.debug(`Tool completed: ${ctx.toolName}`)
    return ctx
  }

  async onAIResponseBefore(context: AIResponseContext): Promise<AIResponseContext> {
    if (!this.toolRouter) return context
    ...
    try {
      ...
      if (context.taskDescription) {
        const recommendations = await this.toolRouter.getPatternRecommendations(
          context.taskDescription,
          context.currentFile
        )
        ;(context.metadata["in-memoria"] as Record<string, unknown>).patterns = recommendations.patterns
      }
    } catch (error) {
  ...
  async onTaskComplete(context: TaskCompleteContext): Promise<void> {
    if (!this.toolRouter || !context.success) return
  ...
  async onAIError(context: AIErrorContext): Promise<void> {
    if (!this.toolRouter) return
  ...
  async onFileChange(context: FileChangeContext): Promise<void> {
    if (!this.toolRouter) return
    if (context.path.includes("node_modules") || context.path.includes(".git")) return
  ...
  async onFileSave(context: FileSaveContext): Promise<void> {
    if (!this.toolRouter) return
  ...
  async onConversationStart(_context: ConversationContext): Promise<void> {
    Logger.info("Conversation started")
  }

  async onConversationEnd(context: ConversationContext): Promise<void> {
    if (!this.toolRouter || !context.summary) return
  ...
  async onToolsList(context: ToolsListContext): Promise<ToolsListContext> {
    if (!this.toolRouter || !context.taskContext) return context
